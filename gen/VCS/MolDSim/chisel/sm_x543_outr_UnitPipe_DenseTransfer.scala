...
/** Hierarchy: x543 -> x444 **/
/** BEGIN Some(DenseTransfer) x543_outr_UnitPipe_DenseTransfer **/
class x543_outr_UnitPipe_DenseTransfer_kernel(
  ...
  val me = this
  val sm = Module(new OuterControl(Streaming, 2, isFSM = false   , latency = 0.0.toInt, myName = "x543_outr_UnitPipe_DenseTransfer_sm")); sm.io <> DontCare
  val iiCtr = Module(new IICounter(1.0.toInt, 2 + _root_.utils.math.log2Up(1.0.toInt), "x543_outr_UnitPipe_DenseTransfer_iiCtr"))
  
  abstract class x543_outr_UnitPipe_DenseTransfer_module(depth: Int)(implicit stack: List[KernelHash]) extends Module {
  ...
  def connectWires0(module: x543_outr_UnitPipe_DenseTransfer_module)(implicit stack: List[KernelHash]): Unit = {
  ...
  def kernel(): Unit = {
    Ledger.enter(this.hashCode, "x543_outr_UnitPipe_DenseTransfer")
    implicit val stack = ControllerStack.stack.toList
    class x543_outr_UnitPipe_DenseTransfer_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x543_outr_UnitPipe_DenseTransfer_module(depth) {
      io.sigsOut := DontCare
      val breakpoints = io.in_breakpoints; breakpoints := DontCare
      val instrctrs = io.in_instrctrs; instrctrs := DontCare
      val rr = io.rr
      val cycles_x543_outr_UnitPipe_DenseTransfer = Module(new InstrumentationCounter())
      val iters_x543_outr_UnitPipe_DenseTransfer = Module(new InstrumentationCounter())
      cycles_x543_outr_UnitPipe_DenseTransfer.io.enable := io.sigsIn.baseEn
      iters_x543_outr_UnitPipe_DenseTransfer.io.enable := risingEdge(io.sigsIn.done)
      Ledger.tieInstrCtr(instrctrs.toList, X543_instrctr, cycles_x543_outr_UnitPipe_DenseTransfer.io.count, iters_x543_outr_UnitPipe_DenseTransfer.io.count, 0.U, 0.U)
      ...
      val x499_inr_Foreach = new x499_inr_Foreach_kernel(List(x468_A_dram), List(x475_fifo), List(x474) ,  Some(me), List(x478_ctrchain), 0, 1, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
      ...
      val x542_outr_Foreach = new x542_outr_Foreach_kernel(List(x475_fifo), List(x476), List(x472_A_sram_1,x471_A_sram_0,x473_A_sram_2) ,  Some(me), List(x502_ctrchain), 1, 2, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
      x542_outr_Foreach.sm.io.ctrDone := (x542_outr_Foreach.cchain.head.output.done).DS(1.toInt, rr, io.sigsIn.backpressure & true.B)
      x542_outr_Foreach.backpressure := true.B | x542_outr_Foreach.sm.io.doneLatch
      x542_outr_Foreach.forwardpressure := (true.B) && (true.B) | x542_outr_Foreach.sm.io.doneLatch
      x542_outr_Foreach.sm.io.enableOut.zip(x542_outr_Foreach.smEnableOuts).foreach{case (l,r) => r := l}
      x542_outr_Foreach.sm.io.break := false.B
      x542_outr_Foreach.mask := ~x542_outr_Foreach.cchain.head.output.noop & true.B
      x542_outr_Foreach.configure("x542_outr_Foreach", Some(io.sigsIn), Some(io.sigsOut), isSwitchCase = false)
      x542_outr_Foreach.kernel()
    }
    val module = Module(new x543_outr_UnitPipe_DenseTransfer_concrete(sm.p.depth)); module.io := DontCare
...
/** END UnitPipe x543_outr_UnitPipe_DenseTransfer **/
