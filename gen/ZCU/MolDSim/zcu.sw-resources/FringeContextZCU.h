...
class FringeContextZCU : public FringeContextBase<void> {
  ...
  void* physToVirt(uint64_t physAddr) {
    ...
    if (iter == physToVirtMap.end()) {
      fprintf(stderr,"Physical address '%lx' not found in physToVirtMap\n. Was this allocated before?", physAddr);
      exit(-1);
    }
    return iter->second;
  }
...
public:
  ...
  FringeContextZCU(string path = "") : FringeContextBase(path) {
    ...
    fprintf(stderr,"placing fringeScalarBase at %lx\n", fringeScalarBase);
    ...
    fprintf(stderr,"placing fringeMemBase at %lx\n", fringeMemBase);
...
//#endif
  ...
  virtual void memcpy(void* hostmem, uint64_t devmem, size_t size) {
    ...
    fprintf(stderr,"[memcpy FPGA -> HOST] hostmem = %p, devmem = %lx, size = %lu\n", hostmem, devmem, size);
  ...
  void flushCache(uint32_t kb) {
    ...
    fprintf(stderr,"[memcpy] dummyBuf = %p, (phys = %lx), arraySize = %d\n", dummyBuf, getFPGAPhys((uint64_t) dummyBuf), arraySize);
    ...
    fprintf(stderr,"[memcpy] dummyBuf = %p, dummyBuf[%d] = %d\n", dummyBuf, arraySize-1, dummyBuf[arraySize-1]);
  }
  ...
  virtual void run() {
    ...
    while((status == 0)) {
      ...
      if (num % 10000000 == 0) {
        ...
        fprintf(stderr,"Elapsed time: %lf ms, status = %08x\n", endTime - startTime, status);
        dumpAllRegs();
...
//    fprintf(stderr, "[readReg] Reading register %d, value = %lx\n", reg, value);
  ...
  void dumpAllRegs() {
    ...
    for (int i=0; i<totalRegs; i++) {
      ...
      if (i < debugRegStart) {
        if (i == 0) fprintf(stderr," ******* Non-debug regs *******\n");
        fprintf(stderr,"\tR%d: %016lx (%08lu)\n", i, value, value);
      } else {
        if (i == debugRegStart) fprintf(stderr,"\n\n ******* Debug regs *******\n");
        fprintf(stderr,"\tR%d %s: %016lx (%08lu)\n", i, signalLabels[i - debugRegStart], value, value);
      }
    }
  }
