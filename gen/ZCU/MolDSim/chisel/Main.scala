...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x669 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x669_p", (x669.bits.addrWidth, x669.bits.sizeWidth))  
    val x670 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x670_p", (x670.bits.v, x670.bits.w))  
    val x671  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    ...
    val RootController = new RootController_kernel(List(x669,x474), List(x476), List(x671), List(x468_A_dram,x470_out_host), List(x670) ,  None, List(), -1, 3, 1, List(1), List(32), breakpoints, rr)
