...
/** Hierarchy: x537 -> x538 -> x444 **/
/** BEGIN None x537_outr_Foreach **/
class x537_outr_Foreach_kernel(
  ...
  val me = this
  val sm = Module(new OuterControl(Sequenced, 2, isFSM = false   , latency = 0.0.toInt, myName = "x537_outr_Foreach_sm")); sm.io <> DontCare
  val iiCtr = Module(new IICounter(1.0.toInt, 2 + _root_.utils.math.log2Up(1.0.toInt), "x537_outr_Foreach_iiCtr"))
  
  abstract class x537_outr_Foreach_module(depth: Int)(implicit stack: List[KernelHash]) extends Module {
  ...
  def connectWires0(module: x537_outr_Foreach_module)(implicit stack: List[KernelHash]): Unit = {
  ...
  def kernel(): Unit = {
    Ledger.enter(this.hashCode, "x537_outr_Foreach")
    implicit val stack = ControllerStack.stack.toList
    class x537_outr_Foreach_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x537_outr_Foreach_module(depth) {
      io.sigsOut := DontCare
      ...
      val x516_inr_UnitPipe = new x516_inr_UnitPipe_kernel(List(b504), List(x475_fifo), List(x507_reg,x505_reg,x506_reg) ,  Some(me), List(), 0, 1, 1, List(1), List(32), breakpoints, rr)
      ...
      val x734_rd_x507 = Wire(new FixedPoint(true, 32, 0)).suggestName("""x734_rd_x507""")
      val x734_rd_x507_banks = List[UInt]()
      val x734_rd_x507_ofs = List[UInt]()
      val x734_rd_x507_en = List[Bool](true.B)
      val x734_rd_x507_shared_en = ((io.sigsIn.forwardpressure).DS(0.0.toInt, rr, io.sigsIn.backpressure & true.B) && ~io.sigsIn.break && (io.sigsIn.datapathEn & io.sigsIn.iiIssue).DS(0.0.toInt, rr, io.sigsIn.backpressure & true.B) && true.B ).suggestName("x734_rd_x507_shared_en")
      x734_rd_x507.toSeq.zip(x507_reg.connectRPort(734, x734_rd_x507_banks, x734_rd_x507_ofs, io.sigsIn.backpressure, x734_rd_x507_en.map(_ && x734_rd_x507_shared_en), true)).foreach{case (a,b) => a.r := b.r}
      val x518_ctr = new CtrObject(Left(Some(0)), Right(x734_rd_x507), Left(Some(1)), 1, 32, false)
      ...
      val x536_inr_Foreach = new x536_inr_Foreach_kernel(List(b504), List(b503), List(x476), List(x472_A_sram_1,x471_A_sram_0,x505_reg,x473_A_sram_2,x506_reg) ,  Some(me), List(x519_ctrchain), 1, 1, 1, List(1), List(32), breakpoints, rr)
      x536_inr_Foreach.sm.io.ctrDone := (x536_inr_Foreach.cchain.head.output.done).DS(1.toInt, rr, io.sigsIn.backpressure & true.B)
      x536_inr_Foreach.backpressure := true.B | x536_inr_Foreach.sm.io.doneLatch
      x536_inr_Foreach.forwardpressure := (x476.valid) && (true.B) | x536_inr_Foreach.sm.io.doneLatch
      x536_inr_Foreach.sm.io.enableOut.zip(x536_inr_Foreach.smEnableOuts).foreach{case (l,r) => r := l}
      x536_inr_Foreach.sm.io.break := false.B
      x536_inr_Foreach.mask := ~x536_inr_Foreach.cchain.head.output.noop & b504
      x536_inr_Foreach.configure("x536_inr_Foreach", Some(io.sigsIn), Some(io.sigsOut), isSwitchCase = false)
      x536_inr_Foreach.kernel()
    }
    val module = Module(new x537_outr_Foreach_concrete(sm.p.depth)); module.io := DontCare
...
/** END UnrolledForeach x537_outr_Foreach **/
